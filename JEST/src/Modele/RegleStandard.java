...
/**
 * The type Regle standard that represent standard rules.
 */
public class RegleStandard implements Regle {
    ...
    public void distribution(ArrayList<Joueur> tabJoueur,int tour,Pioche pioche,Pioche piocheTemp) {
		if(tour == 1) {
			pioche.distribuer(tabJoueur, 2);
		}else {
			piocheTemp.distribuer(tabJoueur, 2);
		}
    }
    ...
    public void gererOffres(ArrayList<Joueur> tabJoueur, Pioche pioche, Pioche piocheTemp) {
		...
		if(pioche.cartes.size() != 0) {
			...
			for(int i = 0; i < tabJoueur.size(); i++) {
				piocheTemp.ajouterCarte(pioche.cartes.remove(0));
			}
		}else {
			for(int i = 0; i < tabJoueur.size(); i++) {
	...
	public void deroulementPartie(List<Joueur> tabJoueur, Paquet paquet) {
		...
		do {
			...
			if (Partie.getGui() != null && tour == 1) {
	            
	            (Partie.getGui()).afficherTrophees(trophy.cartes); 
	        }
			...
			//melange des cartes
            melanger((ArrayList<Joueur>) tabJoueur,tour,pioche,trophy,piocheTemp);
            ...
            distribution((ArrayList<Joueur>) tabJoueur,tour,pioche,piocheTemp);
			
			//Aucun joueur n'a choisi une offre
            reinitAPrisOffre((ArrayList<Joueur>) tabJoueur);

			
			//Procede par tour de jeu 
			//En ce qui concerne le joueur
			tabJoueur.get(0).offre = tabJoueur.get(0).faireOffre();
			if(tabJoueur.get(0) instanceof JoueurPhysique) {
				if (Partie.getGui() != null) {
				    while (Partie.getGui().getCarteFaceCachee() == null) {
				        try {
				            Thread.sleep(100);
            ...
            gererOffres((ArrayList<Joueur>) tabJoueur,pioche,piocheTemp);
